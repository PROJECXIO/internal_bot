...
from internal_bot.bot.services.text_normalizer import make_ngrams, normalize_text, remove_stop_words, tokenize
...
def rank_candidates(
    ...
    query_tokens = remove_stop_words(tokenize(normalized_query))
    query_bigrams = make_ngrams(query_tokens, n=2)
    if not query_tokens and not normalized_query:
        return []

    alias_index = get_alias_index()
    candidate_phrases = [normalized_query, *query_bigrams, *query_tokens]
